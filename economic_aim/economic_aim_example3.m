%1.创建数据
% 定义数据点
y = [10.8934, 10.9669, 11.0795, 11.1729, 11.2489, 11.3117, 11.3647, 11.4101, ...
     11.4499, 11.4852, 11.5169, 11.5169, 11.7280, 11.8543, 11.9444, 12.0134, ...
     12.0684, 12.1134, 12.1510, 12.1830];
u = [9.8437, 6.8294, 5.4561, 4.6380, 4.0871, 3.6884, 3.3835, 3.1419, ...
     2.9436, 2.7774, 2.6353, 2.6353, 1.8317, 1.4471, 1.2076, 1.0408, ...
     0.9171, 0.8215, 0.7452, 0.6831];

% 多项式拟合
p = polyfit(u, y, 3); % 拟合三次多项式
u_fit = linspace(min(u), max(u), 100); % 生成平滑曲线的 x 点
y_fit = polyval(p, u_fit); % 计算对应的 y 值

% 绘制结果
figure;
scatter(u, y, 'b', 'filled'); % 数据点
hold on;
plot(u_fit, y_fit, 'r-', 'LineWidth', 2); % 拟合曲线
title('LQG拟合曲线');
xlabel('u');
ylabel('y');
legend('数据点', '拟合曲线');
...
x0=[490 58.3 12.4742 0];
    ...
    VLB = [490 30 0 0];
    VUB = [500 80 100 100];
zyj=fmincon(f, x0, A, b, Aeq, beq, VLB, VUB,'fitting_constraint3');
