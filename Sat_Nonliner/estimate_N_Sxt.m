function estN = estimate_N(d, Gm, pcase)
    ...
    % 根据不同的pcase选择不同的初始参数向量x0  N=[1+x(1)]/[(1+x(2)+x(3)+...]
    switch pcase
        case 1
            x0 = [-0.2  -0.4 -0.17 0.06 0.01]; % 初始化参数，d-1=4
        case 2
            x0 = [0.6 0.15 -0.5 -0.02 0.05 0.01]; % 初始化参数，d-1=5
        case 3
            x0 = [0 -0.8 0.01]; % 初始化参数d=3
        case 4
            x0 = [-0.02 -0.7 -0.03]; % 初始化参数  分子：x(1)  分母(d-1个)：x(2) x(3) x(4)...
    end
