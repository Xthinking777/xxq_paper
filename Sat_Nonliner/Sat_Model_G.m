...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
sat_alpha_set_1=[10 4.7 3.31 2.6 2.2 1.8 1.1 0.91 0.5 0.1];% 系统白噪声  4.8--10%  3.31--20%  100--0%
% sat_alpha_set_1=zeros(1,10);
% for i=1:1:10
% sat_alpha_set_1(i)=0.9+i*0.01;
% end
KZQ_2 = filt([0.8305 -1.396 0.607], [1 -1]); 
T_2 = filt([zeros(1,6) 1], [1 -0.8]);
N_2 = filt([1 0.6], [1 0.1 -0.67 -0.025 0.105]);
d_2=6;
sat_alpha_set_2=[2.05 1.75 100];% 2.05--10%  1.75--20%
...
g_est=zeros(length(sat_alpha_set),1000);
...
% % 保存最后一列
% last_column = Var_all(:,end);
% % 将除了最后一列的所有列向左移动一位
% Var_all = [last_column,Var_all(:,1:end-1)];

sat_alpha_i=(1:length(sat_alpha_set))';
% 绘制 Var_all 矩阵中的数据
figure; % 创建一个新的图形窗口
plot(sat_alpha_i, Var_all(1, :), 'b-', 'DisplayName', 'Actual Variance');
hold on; % 保持当前图形，以便在同一图上绘制更多数据
plot(sat_alpha_i, Var_all(2, :), 'r--', 'DisplayName', 'Estimated Variance');
plot(sat_alpha_i, Var_all(3, :), 'g-', 'DisplayName', 'Theoretical Variance');
hold off; % 释放图形
